...
\begin{remark}[Crossnorms and \condINJ\ in the Hilbert case]
\label{rem:grothendieck_hilbert}
The crossnorm condition \condCROSS\ and the injective norm condition
\condINJ\ (to be introduced formally in
Definition~\ref{def:crossnorm}) express two levels of compatibility
between the norm $\|\cdot\|_D$ and the tensor product structure. The
injective norm $\|\cdot\|_\varepsilon$ is the smallest reasonable
crossnorm, and \condINJ\ says that $\|\cdot\|_D$ dominates it.

In the Hilbert case, this condition is automatically satisfied and
imposes no effective constraint. More precisely:

\begin{enumerate}
\item[\textup{(i)}]
\textit{All reasonable crossnorms are equivalent on Hilbert tensor
products.}
By the Grothendieck inequality~\cite{Grothendieck1953}, there exists
a universal constant $K_G \leq \pi/(2\ln(1+\sqrt{2})) \approx 1.78$
such that $\|\cdot\|_\pi \leq K_G \|\cdot\|_\varepsilon$ on
$H_1 \otimes H_2$ for any Hilbert spaces $H_1$, $H_2$. Since every
reasonable crossnorm satisfies
$\|\cdot\|_\varepsilon \leq \|\cdot\|_D \leq \|\cdot\|_\pi$, all
reasonable crossnorms on $H_1 \otimes H_2$ are equivalent with
constant $K_G$.
\item[\textup{(ii)}] \textit{The Hilbert tensor norm is the unique norm making
$\VDnorm$ a Hilbert space.} For $H_\alpha = L^2(I_\alpha)$, the
Hilbert tensor norm satisfies $\bigotimes_j L^2(I_j) \cong
L^2(\prod_j I_j)$ isometrically (via the Fubini identification
$\bigotimes f_j \leftrightarrow \prod_j f_j(x_j)$). This is the
natural norm for quantum mechanics and function approximation, and
it satisfies \condINJ\ with equality ($\|\cdot\|_D = \|\cdot\|_H$
dominates $\|\cdot\|_\varepsilon$ by definition of the Hilbert norm).
\end{enumerate}

In contrast, in the Banach case ($V_\alpha = L^p(I_\alpha)$ with
$p \neq 2$), different reasonable crossnorms are genuinely inequivalent,
and the condition \condINJ\ is a non-trivial hypothesis that must be
verified for each choice of $\|\cdot\|_D$. This is one of the reasons
why the Banach-space theory of this monograph is substantially harder
than the Hilbert-space theory of~\cite{KochLubich2007}: in the Banach
case, the geometry depends on the specific choice of norm, not just
the isomorphism class of the spaces.
\end{remark}
...
The tensor product of vector spaces was introduced in its modern form
by Whitney in 1938 and placed within the framework of multilinear
algebra by Bourbaki in the 1940s. The presentation here follows
the approach of Greub~\cite{Greub1981}, which is well suited to the
infinite-dimensional setting: the algebraic tensor product is defined
by the universal property, without any choice of basis or norm.

\paragraph{Crossnorms and Grothendieck's theory.}
The projective and injective tensor norms were introduced by
Grothendieck in his 1953 Résumé~\cite{Grothendieck1953}, which
established the fundamental hierarchy
$\|\cdot\|_\varepsilon \leq \|\cdot\|_D \leq \|\cdot\|_\pi$
for any reasonable crossnorm, identified the dual of
$V_1 \otimes_\pi V_2$ as the space of bounded bilinear forms, and
proved the inequality that now bears his name
(Remark~\ref{rem:grothendieck_hilbert}). This work, largely
unrecognised at the time, became foundational through the notion of
tensor norms and operator ideals. A systematic treatment appears
in~\cite{Hackbusch2019} and~\cite{Ryan2002}.

The condition \condINJ\ is the natural analogue of the
``reasonable crossnorm'' condition in Grothendieck's theory and holds
for all standard function-space tensor products ($L^p$, Sobolev);
see Remark~\ref{rem:crossnorm_examples} in
Chapter~\ref{chap:minsubspaces}.

\paragraph{Tucker format and tensor rank.}
The Tucker decomposition~\cite{Tucker1966} was introduced as a
psychometric tool for three-mode factor analysis. It generalises the
singular value decomposition to higher-order tensors: just as every
matrix $A \in \RR^{m \times n}$ can be written as $A = U \Sigma V^\top$
with orthogonal factor matrices, a Tucker tensor is represented by a
core array and a set of factor matrices in each mode.

The notion of tensor rank for $d \geq 3$ is far more subtle than for
matrices: the maximum rank of a real $n \times n \times n$ tensor is
not known in general, and computing the rank is NP-hard
(H\aa stad, 1990). The minimal-subspace framework of
Chapter~\ref{chap:minsubspaces} avoids these difficulties by working
with the subspace rank $(r_\alpha)_{\alpha \in D}$ rather than the
canonical (CP) rank; the former has a clean algebraic characterisation
and leads to a well-posed approximation problem in Banach spaces.

\paragraph{Tensor product map and the manifold structure.}
The differential of the tensor product map
(Section~\ref{sec:algtensor:tensmap}) is the key ingredient in the
construction of the tangent space to the Tucker manifold. The
formula~\eqref{eq:diff_tensor_product} is the multilinear analogue
of the product rule and appears in~\cite{FHN2019} as the starting
point for the identification
$T_{\bv}\Tucker{\fr}{\VD} \cong \ZD{\bv}$.
