...
Grothendieck in his 1953 Résumé, which also established the fundamental
inequality $\|\cdot\|_\varepsilon \leq \|\cdot\|_D \leq \|\cdot\|_\pi$
for any crossnorm $\|\cdot\|_D$, and identified the dual of
$V_1 \otimes_\pi V_2$ as the space of bounded bilinear forms on
$V_1 \times V_2$. This work, largely unrecognised at the time, later
became the foundation of Banach space theory through the notion of
tensor norms and operator ideals. A systematic treatment appears
in~\cite{Hackbusch2019}, Chapters~3--4.

\paragraph{Crossnorms and tensor product norms.}
The crossnorm condition \condCROSS\ and the injective norm condition
\condINJ\ are formulated in~\cite{FH2012} as the minimal hypotheses
under which the algebraic structure of minimal subspaces extends to
the topological setting. The condition \condINJ\ (the norm dominates
the injective norm) is the natural analogue of the ``reasonable
crossnorm'' condition in Grothendieck's theory and holds for all the
standard function-space tensor products ($L^p$, Sobolev) used in
numerical applications; see Remark~\ref{rem:crossnorm_examples}.
...
singular value decomposition (SVD) to higher-order tensors: just as
every matrix $A \in \RR^{m \times n}$ can be written as
$A = U \Sigma V^\top$ with $U \in \RR^{m \times r}$,
$V \in \RR^{n \times r}$ orthogonal and $\Sigma$ diagonal,
a $d$-th order Tucker tensor is represented by a core array and
a set of factor matrices in each mode.
...
(Håstad, 1990). The minimal-subspace framework of
...
and leads to a well-posed approximation problem.
...
construction of the tangent space to the Tucker manifold. The formula
$D\tau(\bv)[\delta\bv] = \sum_j \tau^{(j)}(\delta v_j)$, where
$\tau^{(j)}$ inserts $\delta v_j$ in the $j$-th slot, is the
multilinear analogue of the product rule and appears in~\cite{FHN2019}
as the starting point for the identification
