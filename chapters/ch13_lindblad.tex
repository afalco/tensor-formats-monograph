...
\begin{proof}
The proof is the quantum specialisation of the proof of
Theorem~\ref{thm:DF_tucker}:
\begin{enumerate}
\item \textit{Smooth manifold.} $\Drank{r}{H}$ is a $\mathcal{C}^\infty$
    Banach manifold by Theorem~\ref{thm:qs:manifold}.
\item \textit{Smooth vector field.} The reduced vector field
    $\rho \mapsto \hat{P}_\rho(\Lind(\rho))$ is locally Lipschitz
    in $\|\cdot\|_1$: $\Lind$ is bounded (Assumption~\ref{ass:lind:regular}(R1)),
    and $\rho \mapsto \hat{P}_\rho$ is locally Lipschitz in operator
    norm because $\hat{P}_\rho = \hat{P}_{\rho}$ depends smoothly on
    $\rho$ via the chart maps~\eqref{eq:qs:chart}
    (Theorem~\ref{thm:qs:manifold}(i)).
\item \textit{ODE on a Banach manifold.} Local existence and uniqueness
    follow from Theorem~\ref{thm:app:ode} (Picard--Lindelöf on Banach
    manifolds). In local chart coordinates $(A(t), \sigma(t))$,
    equation~\eqref{eq:lind:df} becomes
    \begin{equation*}
        \frac{d}{dt}(A(t), \sigma(t))
        = F(A(t), \sigma(t)),
    \end{equation*}
    where $F$ is the pushforward of $\hat{P}_{\rho}\Lind(\rho)$ by the
    inverse chart $\Phi_{S_0}^{-1}$. Since $F$ is locally Lipschitz
    in the Banach space $\cL(\CC^r, S_0^\perp) \times \cTsa{1}(\CC^r)$
    (finite-dimensional in the core direction), Picard--Lindelöf
    applies.
\end{enumerate}
Continuous dependence on $\rho_0$ follows from the same theorem.
\end{proof}
