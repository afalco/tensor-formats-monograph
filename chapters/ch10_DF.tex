...
\label{eq:DF:schrodinger}
    ...
    = \mathcal{H}\,\psi(x_1, x_2, t),
\end{equation}
where $\mathcal{H}$ is the Hamiltonian operator
\begin{equation}
\label{eq:DF:hamiltonian}
    \mathcal{H} = \mathcal{H}_1 \otimes I + I \otimes \mathcal{H}_2
    + V_{12}(x_1, x_2),
\end{equation}
with $\mathcal{H}_\alpha = -\tfrac{1}{2}\Delta_{x_\alpha} + V_\alpha(x_\alpha)$
the single-particle Hamiltonians and $V_{12}$ the interaction potential.

\paragraph{The vector field.}
The vector field is $\bF(\bv) = -i\mathcal{H}\bv$, which is a
...
\label{eq:DF:hartree}
\begin{align}
    i\,\dot\phi_1(t) &= \mathcal{H}_1 \phi_1(t)
    + \langle \phi_2(t), V_{12}\,\phi_2(t)\rangle_{H_2}\,\phi_1(t),
    \label{eq:DF:hartree1}\\
    i\,\dot\phi_2(t) &= \mathcal{H}_2 \phi_2(t)
...
\label{eq:DF:mctdh_core}
    ...
    + (\mu_i^{(1)} + \mu_j^{(2)})\,C^{(D)}_{ij}(t),
\end{equation}
where $\mathcal{H}_\alpha u^{(\alpha)}_i = \mu_i^{(\alpha)}
...
\label{eq:DF:mctdh_basis}
    i\,\dot{u}^{(\alpha)}_j(t)
    = \mathcal{H}_\alpha u^{(\alpha)}_j(t)
    + P^{(\alpha)}_W(t)
