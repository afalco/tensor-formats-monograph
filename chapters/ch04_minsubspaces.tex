...
\begin{equation*}
    \|f - f_r\|_{L^2([0,1]^2)}^2
    = \sum_{k > r} \lambda_k^2
    = \inf_{\bv \in \Tucker{\leq(r,r)}{L^2}} \|f - \bv\|^2.
\end{equation*}
...
elliptic problems: find $u(\cdot, \bmy): D_x \to \RR$ such that
\begin{equation}
\label{eq:parametric_pde}
    -\frac{\partial}{\partial x}
    \Bigl[a(x, \bmy)\,\frac{\partial u}{\partial x}(x, \bmy)\Bigr]
    = f(x),
    \quad x \in D_x,\; \bmy = (y_1, \ldots, y_s) \in \Gamma,
\end{equation}
with homogeneous Dirichlet boundary conditions $u(0,\bmy) =
u(1,\bmy) = 0$. The diffusion coefficient has the affine form
\begin{equation*}
    a(x, \bmy) = a_0(x) + \sum_{j=1}^s y_j a_j(x),
\end{equation*}
where $a_0, a_1, \ldots, a_s \in L^\infty(D_x)$ with
$a(x, \bmy) \geq a_{\min} > 0$ uniformly.

\paragraph{Tensor product setting.}
The solution map $\bmy \mapsto u(\cdot, \bmy)$ defines an element
\begin{equation*}
    \bu \in \mathcal{V} \coloneqq
...
The $\alpha$-rank $r_\alpha(\bu)$ for $\alpha = \{x\}$ (the physical
variable) equals the effective rank of the solution manifold
$\{u(\cdot, \bmy) : \bmy \in \Gamma\} \subset H_0^1(D_x)$:
\begin{equation*}
    \Umin{x}{\bu}
...
the solution $u(\cdot, \bmy)$ varies little with $\bmy$, and
$r_x(\bu)$ is small. More precisely, the Kolmogorov $n$-width
\begin{equation*}
    d_n\bigl(\{u(\cdot,\bmy) : \bmy\in\Gamma\},\,H_0^1(D_x)\bigr)
    \coloneqq \inf_{\substack{W \subset H_0^1(D_x) \\ \dim W = n}}
    \sup_{\bmy \in \Gamma}
    \inf_{w \in W} \|u(\cdot,\bmy) - w\|_{H_0^1}
\end{equation*}
decays at a rate determined by the smoothness of $a$ in the parameters
$\bmy$. When $a$ is analytic in $\bmy$ (which holds when the
...
The best Tucker-$(r,r)$ approximation of $\bu$ in $\mathcal{V}$
is
\begin{equation*}
    \bu_r = \sum_{k=1}^r \lambda_k\, v_k^{(x)} \otimes v_k^{(\bmy)},
    \quad
    \|\bu - \bu_r\|_{\mathcal{V}}^2 = \sum_{k > r} \lambda_k^2,
\end{equation*}
where $\lambda_k$ are the singular values of the solution operator
$\bmy \mapsto u(\cdot, \bmy)$ as a Hilbert--Schmidt operator. The
...
solution manifold, and $v_k^{(\bmy)} \in L^2(\Gamma)$ encode the
