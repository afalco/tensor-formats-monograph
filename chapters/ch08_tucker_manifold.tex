...
For surjectivity: given $\xi \in \ZD{\bv}$ decomposed as
$\xi = \xi_0 + \sum_\alpha \xi_\alpha$ (cf.~Definition~\ref{def:ZD}),
write $\xi_0 = \sum_{(i_\alpha)} \dot{C}^{(D)}_{(i_\alpha)}
\bigotimes_\alpha u^{(\alpha)}_{i_\alpha}$ to determine $\dot{C}^{(D)}$,
and $\xi_\alpha = \sum_{i_\alpha} w^{(\alpha)}_{i_\alpha} \otimes
...
Then $T_\bv\mathfrak{i}(\dot{L}_\alpha, \dot{C}^{(D)}) = \xi$.

Injectivity: if $T_\bv\mathfrak{i}(\dot{L}_\alpha, \dot{C}^{(D)}) = 0$,
then $\xi_0 = 0$ gives $\dot{C}^{(D)} = 0$, and $\xi_\alpha = 0$
